...
from autonomyPkg.srv import SetAutonomy, SetAutonomyResponse
from autonomyPkg.arena_geometry import ArenaGeometry
...
class MissionContext:
    def __init__(self):
        ...
        self.autonomy_service = rospy.Service(
            "/luna/autonomy/toggle",
            SetAutonomy,
            self._handle_autonomy_toggle
        )
    ...
    def systems_ok(self):
        if not self._recent(self.last_costmap_time):
            return False
        if not self._recent(self.last_feedback_time):
            return False
    ...
    def score_cell(self, costmap, x, y):
        ...
        return (
            self.forward_score_weight * float(y)
